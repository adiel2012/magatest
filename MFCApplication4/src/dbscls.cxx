...
int set_member::compare(const char* thatKey, size_t thatSize) const
{
    ...
    return diff == 0 ? this_size - thatSize : diff; 
}
...
void result_set_cursor::select(hnd_t owner, char const* q, nat4 result_set_buf_size, int query_flags, nat4 max_traversed_members)
{
    ...
    storage->query(*this, next_mbr, &query, buf_size, flags, max_members);
}

anyref result_set_cursor::next()
{
    if (i == result.size() && next_mbr) { 
        i = 0;
        do { 
            storage->query(*this, next_mbr, &query, buf_size, flags, max_members);
        } while (next_mbr != 0 && result.size() == 0 && max_members != UINT_MAX);
    } 
    if (i < result.size()) { 
        return result[i++];
    }
...
result_set_cursor::result_set_cursor() : buf_size(0), i(0), next_mbr(0), storage(NULL)
{
}
...
REGISTER(set_owner, object, pessimistic_repeatable_read_scheme);
#endif
...
#ifndef USER_REGISTER
REGISTER(B_tree, set_owner, pessimistic_repeatable_read_scheme);
#if 0
REGISTER_EX(B_page, object, hierarchical_access_scheme,
            class_descriptor::cls_aligned);
#else
REGISTER(B_page, object, hierarchical_access_scheme);
#endif
REGISTER_TEMPLATE(SB_tree16, set_owner, pessimistic_repeatable_read_scheme);
REGISTER_TEMPLATE(SB_page16, object, hierarchical_access_scheme);
#endif
...
#ifndef USER_REGISTER
REGISTER(hash_table, object, pessimistic_repeatable_read_scheme);
REGISTER(hash_item, object, hierarchical_access_scheme);
#endif
...
#ifndef USER_REGISTER
REGISTER(H_page, object, hierarchical_access_scheme);
#endif
...
H_tree::H_tree(size_t hash_size, class_descriptor& desc) : object(desc) 
{ 
...
#ifndef USER_REGISTER
REGISTER(H_tree, object, pessimistic_repeatable_read_scheme);
#endif
...
REGISTER(ExternalBlob, object, optimistic_scheme);
#endif
...
#ifndef USER_REGISTER
REGISTER_ABSTRACT(KD_tree, object, pessimistic_repeatable_read_scheme);
REGISTER(KD_tree::Node, object, hierarchical_access_scheme);
#endif
