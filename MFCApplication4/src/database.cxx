...
void obj_storage::query(result_set_cursor& cursor, opid_t& next_mbr, char const* query, nat4 buf_size, int flags, nat4 max_members)
{
    critical_section guard(cs);
    assert(opened);

    storage->query(next_mbr, query, buf_size, flags, max_members, loadBuf);
...
void obj_storage::load(hnd_t *hnds, int num_hnds, int flags)
{
    ...
    opid_t *opids = (opid_t *)opidsBuffer.append(num_hnds * sizeof(opid_t));
...
//
boolean obj_storage::lock(opid_t opid, lck_t lck, int attr)
{
    boolean result;
...
opid_t obj_storage::allocate(cpid_t cpid, size_t size, int flags, hnd_t cluster_with)
{ 
    int buf_size = db->alloc_buf_size;
    ...
    if (buf_size != 0) { 
        ...
        if (alloc_buf_size == 0) {
            object_monitor::unlock_global();
            {
                critical_section guard(cs);
                if (alloc_buf_size == 0) { 
                    alloc_opid_buf = new opid_t[buf_size];
...
void obj_storage::deallocate(opid_t opid)
{
    for (int n = alloc_buf_pos, i = n; --i >= 0;) { 
...
void obj_storage::invalidate(stid_t, opid_t opid)
{
    object_monitor::lock_global(); 
...
{ 
    ...
    n_references = 0;
    if (!storage->open(connection_address, login, password)) { 
        delete storage; 
        storage = NULL; 
        return False;
    }
...
void database::get_object(object_ref& r, opid_t opid, stid_t sid)
{
    object_monitor::lock_global();
