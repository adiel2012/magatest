    ...
    while (true) { 
        ...
        if (strncmp(buf, "GET ", 4) == 0) { 
            ...
            if (stristr(buf, "Content-type: multipart/form-data")) {
                ...
                char*    data    = NULL;
                int      dataLen = 0;
                data = con.readHTTPbody(buf, inputBufferSize, size, body, dataLen,
                                         0, NULL, true, &biggie);
                if (data) {
...
ScanNextPart:
            ...
            data = con.readHTTPbody(buf, inputBufferSize, size, p, dataLen,
                0, NULL, true);
...
// if patern not found.
//*******************************************************************************
char *HTTPapi::BinaryPatternSearch (char* iStart, char* iEnd, char *iPattern, size_t iPatternSize)
{
...
//*******************************************************************************
char *HTTPapi::extractTokenValue (char *Token, char *start, char *end)
{
                           ...
                           size_t *o_file_size, nat2 *o_http_code)
{
    ...
    while (totalRead < file_size) {
        ...
        if (numRead <= 0) {
            fclose(f);
            con.reset();
            *err = ERROR_TEXT("500 Internal server error");
            if (o_http_code != NULL) {
                *o_http_code = 500;
            }
            return False;
...
boolean HTTPapi::handlePut(WWWconnection& con, char* header, char* host,
                           big_body& content)
{
