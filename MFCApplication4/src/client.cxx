...
opid_t dbs_client_storage::allocate(cpid_t cpid, size_t size, int flags, opid_t clusterWith)
{ 
    dbs_request snd_req, rcv_req; 
    snd_req.cmd = dbs_request::cmd_alloc;
    snd_req.alloc.cpid = cpid;
    snd_req.alloc.size = size;
    snd_req.alloc.flags = (nat1)flags;
    snd_req.alloc.cluster = clusterWith;
                                       ...
                                       opid_t opidBuf[], size_t nReservedOids, hnd_t clusterWith[])
{
    ...
    req->bulk_alloc.n_objects = nAllocObjects;
    req->bulk_alloc.reserved = nReservedOids;
...
cpid_t dbs_client_storage::put_class(dbs_class_descriptor* dbs_desc)
{
    ...
    put_class_req->clsdesc.size = dbs_desc_size;
...
void dbs_client_storage::send_messages() {
  ...
  req->object.extra = send_msg_buf.size() - 1;
...
void dbs_client_storage::query(opid_t& next_mbr, char const* query, nat4 buf_size, int flags, nat4 max_members, dnm_buffer& buf)
{
    ...
    snd_req->query.query_len = query_len;
    snd_req->query.member_opid = next_mbr;
    snd_req->query.max_buf_size = buf_size;
    snd_req->query.max_members = max_members;
    memcpy(snd_req+1, query, query_len);
    send_receive(snd_req, sizeof(dbs_request) + query_len, rcv_req, dbs_request::cmd_query_result, buf);
    next_mbr = rcv_req.result.tid;
    if (!rcv_req.result.status) { 
        throw QueryException(&buf);
    }
}

void dbs_client_storage::load(opid_t opid, int flags, dnm_buffer& buf)
{
    dbs_request snd_req, rcv_req;
...
void dbs_client_storage::load(opid_t* opp, int n_objects, 
...
void dbs_client_storage::forget_object(opid_t opid)
{
    dbs_request snd_req;
...
void dbs_client_storage::throw_object(opid_t opid)
{
    dbs_request snd_req;
                                                           ...
                                                           trid_t& tid)
{
    ...
    trans_req->trans.size = buf.size() - sizeof(dbs_request);
    trans_req->trans.crc = calculate_crc(trans_req+1, buf.size() - sizeof(dbs_request));
...
boolean dbs_client_storage::open(const char* server_connection_name, const char* login, const char* password) 
{
...
void dbs_client_storage::add_user(char const* login, char const* password)
{
    ...
    req->any.arg3 = strlen(login)+1;
    req->any.arg4 = strlen(password)+1;
