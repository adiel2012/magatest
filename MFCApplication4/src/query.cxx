...
char* Runtime::unpackStruct(char* &refs, char* bins, size_t size, dbs_class_descriptor* desc, char* name, int fno, bool skip)
{
    ...
    do { 
        ...
        if (!skip && (no != 0 || field->type != fld_structure)) { 
        ...
        if (field->n_items == 0) { // varying part
            ...
            case fld_structure:
                if (fno > no+1 || (fno == 0 && no+1 < (int)desc->n_fields)) {
                    if (no == 0) { // first struct used to represent base class: let access it without class name prefix
                        bins = unpackStruct(refs, bins, size, desc, name, 1, false);
                    } else { 
                        strcat(name, ".");
            ...
            case fld_string:
            {
                ...
                if (binding != NULL) { 
                    if (size != 0xFFFF) { 
                        ...
                        wchar_t* chars = str.getWideChars();
