...
void client_agent::execute_query(dbs_request const& req)
{
    ...
    if (req.query.query_len != 0) { 
        ...
        try { 
            ...
            reply->result.size = msg_size + 1; // include terminator character
            reply->result.tid = 0;
    ...
    dnm_buffer mbr_buf;
    dbs_handle hnd;
    opid_t member_opid = req.query.member_opid;
    ...
    buf.put(cluster_size < max_buf_size ? cluster_size : max_buf_size); // reserve enough space in buffer
    buf.put(sizeof(dbs_request));
    
    server->obj_mgr->load_object(member_opid, lof_bulk, this);
    server->mem_mgr->get_handle(member_opid, hnd);
    size_t size = hnd.get_size();
    cpid_t cpid = hnd.get_cpid();
    assert(size != 0 && cpid != 0);
    ...
    server->obj_mgr->release_object(member_opid);
    
    opid_t next_opid;
    stid_t next_sid;
    unpackref(next_sid, next_opid, &mbr_buf + next_field_offs);
    
    for (size_t i = 0; buf.size() < max_buf_size && i < max_members; i++) { 
        if (next_sid != self_sid || next_opid == 0) {
            member_opid = 0; // no more members
            break;
        }
...
boolean storage_server::backup()
{
    ...
    if (backup_file_name_length > 3 && stricmp(backup_file_name+backup_file_name_length-4, ".inc") == 0) {
        ...
        if (inc != NULL) {             
            while (fgets(buf, sizeof buf, inc) != NULL) { 
                nat4 last_backup_no;
                if (sscanf(buf, "%u %" INT8_FORMAT "u", &last_backup_no, &last_page_timestamp) != 2 || last_backup_no != backup_no) {
        ...
        char* backup_part_name = new char[backup_file_name_length + 8];
        sprintf(backup_part_name, "%.*s-%u.bck", backup_file_name_length-4, backup_file_name, backup_no);
    ...
    if (incremental_backup) { // incremental backup
        ...
        fprintf(inc, "%u %" INT8_FORMAT "u\n", backup_no, last_page_timestamp);
        fclose(inc);
    }   
    cs.leave();
    if (backup_callback) {
                                ...
                                const char* database_config_file)
{
    ...
    if (backup_file_name_length > 3 && stricmp(backup_file_name+backup_file_name_length-4, ".inc") == 0) {
        ...
        while (fgets(buf, sizeof buf, inc) != NULL) { 
            ...
            if (sscanf(buf, "%u %" INT8_FORMAT "u", &backup_no, &page_timestamp) != 2 || last_backup_no+1 != backup_no || page_timestamp < last_page_timestamp) {
                console::message(msg_error|msg_time, "Corrupted incremental backup header file %s\n", backup_file_name);
                fclose(inc);                    
                return false;
            }
