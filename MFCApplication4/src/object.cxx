...
{ 
  ...
  public:
    ...
    hnd_t find(obj_storage* storage, opid_t opid) { 
        for (hnd_t hnd = table[opid % size]; hnd != 0; hnd = hnd->next) { 
    ...
    void remove(hnd_t hnd) { 
        used -= 1;
        hnd_t hp, *hpp = &table[hnd->opid % size]; 
        while ((hp = *hpp) != hnd) { 
            hpp = &hp->next;
        }
        *hpp = hnd->next;
...
void object_handle::dump(void)
{
    ...
    console::output("Object %s:%08lx at (%08lx) of class \"%s\" "
                    "has %ld references.\n", storageID, opid, (nat4)obj, type,
                    access);
}
...
void object_handle::make_persistent(hnd_t hnd, obj_storage* parent_storage)
{
    ...
    opid_t opid = hnd->storage->allocate(obj->cpid, desc->packed_size((char*)obj, obj->size), flags, obj->cluster);
...
{ 
    return hnd->storage ? hnd->storage->db : (database const*)0; 
}
...
class_descriptor object::self_class("object", 
                                    ...
                                    &optimistic_scheme, 
                                    &object::constructor,
                                    &((object*)0)->object::describe_components(), 
                                    NULL);
