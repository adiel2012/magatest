...
class GOODS_DLL_EXPORT obj_storage : public dbs_application { 
  ...
  public: 
    ...
    //
    void    forget_object(opid_t opid) { 
        if (opened) { 
    ...
    //
    opid_t  allocate(cpid_t cpid, size_t size, int flags, hnd_t cluster_with); 

    //
    // Deallocate object at server
    //
    void    deallocate(opid_t opid);
    ...
    // 
    void    query(result_set_cursor& cursor, opid_t& next_mbr, char const* query, nat4 buf_size, int flags, nat4 max_members);
...
class GOODS_DLL_EXPORT database { 
  ...
  protected: 
    ...
    obj_storage*    get_storage(stid_t sid) {
  ...
  public:
    ...
    //
    void get_object(object_ref& ref, opid_t opid, stid_t sid = 0);
