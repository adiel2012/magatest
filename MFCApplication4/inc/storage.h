...
class GOODS_DLL_EXPORT dbs_application { 
  public: 
    ...
    //
    virtual void invalidate(stid_t sid, opid_t opid) = 0;
    //
...
class GOODS_DLL_EXPORT dbs_storage { 
  ...
  public: 
    virtual opid_t  allocate(cpid_t cpid, size_t size, int flags, opid_t clusterWith) = 0;
    virtual void    bulk_allocate(size_t sizeBuf[], cpid_t cpidBuf[], size_t nAllocObjects, 
                                  opid_t opidBuf[], size_t nReservedOids, hnd_t clusterWith[]) = 0;
    ...
    virtual void    query(opid_t& next_mbr, char const* query, nat4 buf_size, int flags, nat4 max_members, dnm_buffer& buf) = 0;
    ...
    //
    virtual boolean open(const char* server_connection_name, char const* login = NULL, char const* password = NULL) = 0;
