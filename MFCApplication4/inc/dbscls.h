...
class array_template : public object { 
  protected: 
    ...
    array_template* set_size(nat4 n) { 
        nat4 allocated = (size - this_offsetof(array_template, array)) / sizeof(T); 
        if (n > allocated) { 
...
class GOODS_DLL_EXPORT String : public ArrayOfByte { 
  ...
  public: 
    ...
    void append(const char* tail) { 
        ...
        String* s = (String*)set_size(n+len);
        for (size_t i = 0; i < len; i++) { 
            s->array[i+n] = tail[i];
        }
    }   
    ...
    void replace(const char* str) { 
        String* s = (String*)set_size(strlen(str)+1);
        strcpy(s->array, str);
    }
...
class GOODS_DLL_EXPORT result_set_cursor
{
  ...
  public:
     ...
     */
    anyref next();
    void select(hnd_t owner, char const* query, nat4 buf_size = UINT_MAX, int flags = qf_default, nat4 max_members = UINT_MAX);
...
class GOODS_DLL_EXPORT hash_table : public object { 
  public: 
    //
    ...
    size_t get_numer_of_elements() const { return n_elems; }

    static ref<hash_table> create(size_t size) {
        return new (self_class, size) hash_table(size);
    }
    METACLASS_DECLARATIONS(hash_table, object);
...
class GOODS_DLL_EXPORT H_tree : public object { 
  public: 
    //
    // Add to hash table association of object with specified name.
    //
    ...
    METACLASS_DECLARATIONS(H_tree, object);
...
{ 
  ...
  public:
    ...
    ExternalBlob(void* buf, size_t buf_size) : object(self_class, buf_size) 
    { 
        memcpy(data, buf, buf_size);
    }

    static ref<ExternalBlob> create(void* buf, size_t buf_size) { 
