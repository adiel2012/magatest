...
enum transaction_object_flags {
    ...
    tof_unlock_exl = 8, // request to release exclusive lock at the end
                        // of transaction
    tof_closure    = 16,// object is part of clusure: is referenced from explicitely requested object
    tof_change_metadata = 32// transaction updating class defintion
};
...
enum load_object_flags { 
    ...
    lof_auto     = 4, // user storage defaults
...
enum alloc_object_flags { 
    aof_none      = 0,
    aof_aligned   = 1, // algign object on page boundary
...
struct dbs_object_header { 
    ...
    nat2  cpid;      // class indentifier for store
    ...
    opid_t get_cpid()          { return unpack2((char*)&this->cpid); }
...
enum lck_t { 
    lck_none,      // No lock
    lck_shared,    // Prevent object from been modified or exclusivly 
                   // locked by other processes
    lck_exclusive, // Prevent object from been modified or locked
                   // (shared or exlusive) by other processes
    lck_temporary_exclusive, // exclusive lock which is hold until release is perfromed and then downgraded to shared
...
enum query_flags {
    qf_default         = 0,
    qf_backward        = 1, // backward traveral of set members
